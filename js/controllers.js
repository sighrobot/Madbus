...
function MainCtrl($scope, $rootScope, $window, $location, Page) {
  ...
  'page': $location.path()
	...
	Page.setTitle('Realtime bus arrival estimates');
}
...
app.controller('RouteListCtrl', function ($scope, routesService, Page) {
	...
	routesService.getRoutes(0)
	.then(function(data){
		$scope.routes = data;
	});

	Page.setTitle('Find stops by route');

	$scope.rangeMin = 0;
	$scope.rangeMax = 100;

	$scope.setRange = function(a,b) {
		$scope.rangeMin = a;
		$scope.rangeMax = b;
		$scope.query1 = '';
	}

	$scope.rangeFilter = function(r) {
		return r[2] >= $scope.rangeMin && r[2] <= $scope.rangeMax;
	}

});


app.controller('StopListCtrl', function ($scope, $routeParams, stopsByRouteService, routesService, Page) {

	$scope.routeID = $routeParams.routeID;

	stopsByRouteService.getStopsByRoute()
	.then(function(data){
		$scope.stops = data;
		for (var i in $scope.stops) {
						$scope.markerLat = $scope.stops[i].latitude;
						$scope.markerLng = $scope.stops[i].longitude;
						var info = '<a href="#/stop/' +
						$scope.stops[i].stopID +
						'"><h5 style="white-space:nowrap;margin-bottom:0;">' +
						$scope.stops[i].intersection +
						'</h5></a><p style="margin-top:0;"><strong>' +
						$scope.stops[i].destination +
						'</strong><br><span class="muted">Stop ID: <strong>' +
						$scope.stops[i].stopID +
						'</strong></span></p>';
						$scope.addMarker(info);
						$scope.markerNum++;
					}
	});

	routesService.getRoutes($scope.routeID)
	.then(function(data){
		$scope.bgColor = data[0][0];
		$scope.color = data[0][1];
		$scope.desc = data[0][2];
	});
	...
	Page.setTitle('Route ' + $scope.routeID.dezero());
...
/*$scope.setPrimaryMarker = function(lt, ln) {
	lt = parseFloat(lt);
	ln = parseFloat(ln);
	$scope.active = {
		lat: lt,
...
$scope.removePrimaryMarker = function(lt, ln) {
	lt = parseFloat(lt);
	ln = parseFloat(ln);
	$scope.active = {
		lat: 0,
...
app.controller('ArrivalCtrl', function ($scope, $sce, $routeParams, $timeout, arrivalsService) {
			...
			if (data.status == -1) {
				...
				for (var i in data.stop.route) {
					...
					if (data.stop.route[i].destination.length == 2) {
						data.stop.route[i].destination[1] = 'via ' + data.stop.route[i].destination[1];
					}
				}
				$scope.rList.sort();
			}
	...
	$scope.calcETA = function(m) {
		if (m < 60) return $sce.trustAsHtml('<strong>' + m + '</strong>m');
		var h = Math.floor(m / 60);
		if (m % 60 == 0) return $sce.trustAsHtml('<strong>' + h + '</strong>h');
		else {
			return $sce.trustAsHtml('<strong>' + h + '</strong>h&nbsp;<strong>' + (m % 60) + '</strong>m');
		}
	}
